...
// App — composition root сервиса gateway.
type App struct {
	cfg  *config.Config
	http *http.Server
}
...
func (a *App) Run(ctx context.Context) error {
	...
	g.Go(func() error {
		fmt.Printf("%s: HTTP на %s\n", a.cfg.ServiceName, a.http.Addr)
		fmt.Printf("%s: upstream auth=%s account=%s transaction=%s notifications=%s\n",
			a.cfg.ServiceName, a.cfg.AuthGRPCAddr, a.cfg.AccountGRPCAddr, a.cfg.TransactionGRPCAddr, a.cfg.NotificationsGRPCAddr)
